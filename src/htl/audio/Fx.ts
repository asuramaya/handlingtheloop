...
export class FxRack {
  ...
  readonly input: GainNode;
  /** ★ VOICES THAT NO STEM TAP CARRIES — the scratch resampler, the sampler's deck-route pads,
   *  the mic. They must enter HERE, not at `input`.
   *
   *  `input` is the deck's main source, and once stems are separated the rack does not read it any
   *  more: each chain is fed from the stretch worklet's per-stem TAPS, and the taps already carry
   *  that same signal. So `chainIn` is deliberately left unconnected whenever a live tap exists —
   *  connecting it would play the track twice.
   *
   *  Which silently took the injected voices down with it. A scratch is generated by its own
   *  worklet and appears on NO tap; with a stem chain built and stems live it entered `input` and
   *  went nowhere, so a scrub faded the deck's source down and faded nothing in — the scratch you
   *  heard was leakage, which is exactly the "scratching sounds worse" report that found this.
   *  Same for a sampler region pad and for mic-to-deck.
   *
