...
export const FACTORY_PRESETS: Record<string, FxPreset[]> = {
  ...
  //     a vocal: the tails live in the gaps instead of smearing the words.
  //   • `timeMode` — what a TIME change does to a ringing tail. 0 Repitch (tape slur — the DJ
  ...
  reverb: [
    ...
    { name: "Dark Chamber", params: { mix: 0.3, size: 0.75, decay: 0.7, brightness: 0.3, predelay: 0.02, width: 1, lowCut: 120, highCut: 6000, drive: 0.45, character: 0.2, modRate: 0.3, postLow: 3, postHigh: -6, duck: 0, freeze: 0, style: 0 } },
    ...
    // FREEZE was dead: the tank stops taking input and holds what it has, forever. Held on a pad
    // it's an infinite sustain you can mix a new track in under — the reverb's biggest single trick.
    { name: "Freeze Tank", params: { mix: 0.5, size: 0.85, decay: 0.9, brightness: 0.6, predelay: 0.01, width: 1.2, lowCut: 200, highCut: 11000, drive: 0, character: 0.2, modRate: 0.4, postLow: 0, postHigh: 0, duck: 0, freeze: 1, style: 0 } },
  ],
  ...
  // Two of MOD's powers were dead, and they're the two that separate it from a stompbox:
  //   • `sync:1` — the LFO locks to the deck. `rate` then indexes MOD_DIVS (4 bar…1/8) instead of
  //     free Hz, so ext = i/5: 0.0 = 4 bar, 0.4 = 1 bar, 0.8 = 1/4. A flanger sweep that lands with
  //     the phrase is a different instrument from one that drifts against it.
  //   • `src` — LFO (0) / ENV (1) / BOTH (2). On ENV the sweep is driven by the TRACK's envelope,
  //     so the filter moves with the drums instead of on its own clock (auto-wah/env-phaser).
  // The free `rate` also only ever sat between 0.40 and 0.60 — the middle fifth of its travel. The
  // slow-drift and fast-warble ends of the knob are where the character lives.
  mod: [
    { name: "Lush Chorus", params: { mix: 0.45, mode: 0, rate: 0.35, depth: 0.5, feedback: 0.2, tone: 0.5, stages: 6, wave: 0, src: 0, thru: 0, sync: 0 } },
    { name: "Wide Ensemble", params: { mix: 0.5, mode: 0, rate: 0.22, depth: 0.7, feedback: 0.1, tone: 0.55, stages: 6, wave: 0, src: 0, thru: 0, sync: 0 } },
    { name: "Jet Flanger", params: { mix: 0.5, mode: 1, rate: 0.5, depth: 0.6, feedback: 0.6, tone: 0.5, stages: 6, wave: 0, src: 0, thru: 0, sync: 0 } },
    { name: "Synced Flanger", params: { mix: 0.5, mode: 1, rate: 0.4, depth: 0.85, feedback: 0.65, tone: 0.5, stages: 6, wave: 0, src: 0, thru: 0, sync: 1 } },
    { name: "Through-Zero", params: { mix: 0.5, mode: 1, rate: 0.55, depth: 0.75, feedback: 0.5, tone: 0.5, stages: 6, wave: 0, src: 2, thru: 1, sync: 0 } },
    { name: "Env Phaser", params: { mix: 0.55, mode: 2, rate: 0.5, depth: 0.8, feedback: 0.5, tone: 0.5, stages: 6, wave: 0, src: 1, thru: 0, sync: 0 } },
    { name: "Slow Phaser 8", params: { mix: 0.5, mode: 2, rate: 0.12, depth: 0.8, feedback: 0.6, tone: 0.45, stages: 8, wave: 0, src: 0, thru: 0, sync: 0 } },
    { name: "Fast Vibrato", params: { mix: 0.6, mode: 0, rate: 0.85, depth: 0.9, feedback: 0, tone: 0.5, stages: 6, wave: 0, src: 0, thru: 0, sync: 0 } },
    { name: "Square Phaser", params: { mix: 0.5, mode: 2, rate: 0.8, depth: 0.7, feedback: 0.3, tone: 0.5, stages: 4, wave: 2, src: 0, thru: 0, sync: 1 } },
  ],
  ...
  gate: [
    ...
    // sync:0 — the rate leaves the grid and becomes free Hz. That isn't a beat-chop, it's a tremolo.
    { name: "Free Tremolo", params: { mix: 1, rate: 0.45, depth: 0.8, duty: 0.5, smooth: 0.35, shape: 4, sync: 0 } },
  ],
  ...
  eq: [
    ...
    // The band SHAPES were dead — LOW and HIGH can be BELLS, not just shelves, and a bell has a Q.
    // That's the difference between "turn the bass up" and a resonant bump you can slam on a drop.
