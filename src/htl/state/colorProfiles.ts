...
export function createColorProfile(name: string, colors: ColorSnapshot): ColorProfile {
  return { id: uid("c"), name: name.trim() || "Untitled theme", colors: { ...colors }, updatedAt: Date.now() };
}

export function duplicateColorProfile(p: ColorProfile): ColorProfile {
  return { ...p, id: uid("c"), name: `${p.name} copy`, colors: { ...p.colors }, updatedAt: Date.now() };
}
...
export function parseColorProfile(text: string): ColorProfile | null {
  return parseEnvelope<ColorProfile>(EXPORT_KIND, "profile", text, (raw) => {
    const p = raw as Partial<ColorProfile>;
