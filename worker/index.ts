...
export function ogBlock(o: { title: string; desc: string; img: string; url: string; large: boolean }): string {
  const tags = [
...
export function defaultOgMeta(url: URL): string {
  return ogBlock({
    ...
    // `img` was "" here, which meant the homepage card carried a title and a description and NO
    // PICTURE — ogBlock's `large && img` guard then correctly downgraded it to a small `summary`
    // card. So every paste of the bare domain unfurled as a text-only link while every /@handle
    // and /set/ link got a picture. og.png is the site's own card, generated by
    // scripts/make-og-image.py from the real favicon mark and the landing page's own palette.
    img: `${url.origin}/og.png`,
    url: `${url.origin}/`,
    large: true,
  });
}
